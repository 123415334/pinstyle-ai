...
async function fetchSmallBase64(url) {
  try {
    ...
    const resp = await fetch(url, {
      ...
      headers: { "User-Agent": "Mozilla/5.0", ...(isPinterest ? { "Referer": "https://www.pinterest.com/" } : {}) },
    });
...
module.exports = async function handler(req, res) {
  ...
  try {
    const downloaded = await Promise.all(imageUrls.slice(0, 2).map(fetchSmallBase64));
      ...
      const claudeResp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: [
            { type: "text", text: "Analyze these images. Return ONLY a comma-separated list of style descriptors: rendering technique, lighting, colors, texture, mood. No explanation." },
    ...
    // Wait 12 seconds between predictions to avoid rate limit burst
